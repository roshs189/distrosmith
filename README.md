...
- **`distro-smith`** — the combined end-to-end flow: runs
  ...
  `meta-qcom` PR. Writes the run's outcome to `distro-params.yaml`. This
  orchestration is currently **meta-qcom-specific** (not
...
All three skills — and `setup.py` — are driven by three env vars. Nothing else

...
description: >-
  ...
  those two files to a new `add/<machine>` branch and opens a PR against the
  ...
  required. The PR is left open rather than auto-merged; a caller (e.g. the
  `distro-smith` orchestrator) points meta-qcom's `SRCREV` at the PR's head
  commit directly. meta-qcom regenerates the real flashable artifacts at build
...
artifacts, runs `qcom-ptool` to actually produce them, then opens (but does
not merge) a PR with just the source files against the configured
qcom-ptool fork. The board-spec entry is the only source of partition facts this skill trusts
...
- **`BUILD_DISTRO_ROOT`** (optional, defaults to
  ...
  flow's `distro-params.yaml` is written outside the work directory.
...
## 6. Commit and open the PR (do not merge)
...
- A `201` response with an `html_url` field means the PR was created —
  note both `html_url` and `head.sha` (needed by a caller bumping
  `qcom-ptool.inc`'s `SRCREV`).
...
Leave the PR open — this skill never merges it. Report the PR's `html_url`
and `head.sha` (the exact commit a caller such as `distro-smith` bumping
`qcom-ptool.inc`'s `SRCREV` should point at — see Section 7) back to the
user/caller. Merging the fork PR (whether by the user reviewing it by hand,
or automatically once meta-qcom's build against that SRCREV succeeds) is
...
qcom-ptool checkout, and are open as a PR against the fork." Getting them
into an actual `qcomflash` build output additionally requires, as separate
follow-up work — automated by `distro-smith` for items 2-4 below (item 1's
fork-PR merge is a manual decision even under `distro-smith`, see that
skill's own SKILL.md), but confirm with the user first if running
standalone, since it touches what gets built:

1. Merging the fork PR (Section 6, left open by this skill) — either by
   hand after review, or automatically once a caller's build against the
   PR's head SHA (item 2 below) succeeds. Then merging that fork commit
   upstream into `qualcomm-linux/qcom-ptool` (its own PR against `main`, per
   `CONTRIBUTING.md`: `make lint`, `make generate-checksums` for the new
   files, `make all integration check-checksums` with
   `PTOOL_SEED=qcom-ptool-ci` set) — the fork PR this skill opens is
   a staging step, not the final upstream contribution, unless the user's
   workflow treats the fork as authoritative. Both remain manual.
2. Bumping `SRCREV` in `meta-qcom`'s `recipes-bsp/partition/qcom-ptool.inc`
   to the fork PR's head commit from Section 6 (with `SRC_URI`'s `git://`
   fetcher given `nobranch=1`, since that commit isn't necessarily on
   `main` yet) — the `distro-smith` orchestrator does this automatically
   when given that commit SHA (see that skill's own SKILL.md);
   `qcom-yocto-new-machine` itself no longer performs this step.
...
- Report back exactly which files were written/generated, the
  `check-missing-files` result, and the created (still-open) PR's URL and
  head SHA; don't just say "done."
